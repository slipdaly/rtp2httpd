...
# Process rtp2httpd-minimal Makefile
awk -v version="$RELEASE_VERSION" -v pkg_hash="$PKG_HASH" '
  # Skip comment lines about version extraction and Makefile.versioned
  /^# Extract version from git tags/ { next }
  /^# For firmware maintainers:/ { next }
  ...
  # Print all other lines
  { print }
' openwrt-support/rtp2httpd-minimal/Makefile > openwrt-support/rtp2httpd-minimal/Makefile.versioned
...
echo ""
echo "=== rtp2httpd-minimal/Makefile.versioned ==="
cat openwrt-support/rtp2httpd-minimal/Makefile.versioned
echo ""
